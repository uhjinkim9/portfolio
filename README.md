...
분산된 협업과 업무 관리 기능을 하나의 서비스로 통합하도록 설계한 MSA 기반 그룹웨어 플랫폼입니다.

- 11개 마이크로서비스 중 유사 도메인을 통합하고 실행 구성과 DB 커넥션 구조를 점검해 개발 환경의 자원 부담 완화
- RabbitMQ를 활용해 메인 API와 알림·메일 후처리 로직 분리
- 인사 담당자와 협업해 연차·반차 및 근태 인정 범위의 기본·예외 규칙을 정리하고, 근태·연차 프로세스를 그룹웨어에 자동 연동
- 메신저의 화면 위치와 필요 기능을 정의하고 AI를 활용해 1:1 채팅·파일 첨부·이미지 붙여넣기 MVP를 **3일 내 구현**
- 주제별 개발 가이드와 참조 경로를 인덱스화해 AI가 필요한 문서만 단계적으로 읽도록 구성하고 컨텍스트 사용 효율 개선
- 인프라팀의 가이드에 따라 Helm 기반 GitOps 배포 구성을 작성하고 `development → stage → production` 브랜치 흐름 정립
- 첫 GitOps 구축 과정에서 AI를 활용해 설정과 오류를 점검하고 Kubernetes 환경 배포 및 가오픈 완료
...
- 지인 VOC를 반영해 구체 메뉴, 생리 일차, 수면 및 과식 여부 기록 기능 추가
- 식단 상관관계와 생활 조건별 배변 상태를 비교하는 기간별 통계·분석 기능 확장
- Device ID 기반 기존 사용자를 유지하면서 OIDC 통합 계정 생성을 안내하고, 사용자 동의 후 기존 기록을 연결하는 점진적 인증 전환 구현
